...
At first, install NestJS packages, [`@agentica/core`](/docs/core) and [`@samchon/openapi`](https://github.com/samchon/openapi). `@agentica/core` is the core library of Agentica Framework, and `@samchon/openapi` is a library defining LLM function calling schemas including their converters from the Swagger/OpenAPI documents.
...
export namespace start {
  ...
  export type Listener = IAgenticaRpcService;
