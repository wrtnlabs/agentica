...
At last, run `npx typia setup` command. It's because `typia` is a transformer library analyzing TypeScript source code in the compilation level, and it needs additional setup process transforming TypeScript compiler via [`ts-patch`](https://github.com/nonara/ts-patch). If you're not using non-standard TypeScript compiler (not `tsc`), you have to rollback to the standard TypeScript compiler, or setup [`@typia/unplugin`](https://typia.io/docs/setup/#typiaunplugin) following its guidance.
...
await server.open(3_001, async (acceptor) => {
  ...
  const service: AgenticaRpcService = 
