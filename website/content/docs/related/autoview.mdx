...
To generate frontend code from a TypeScript type, utilize the [`typia.llm.parameters<Schema>()`](https://typia.io/docs/llm/parameters/) function.

Create an `AutoViewAgent` instance with [`typia.llm.parameters<Schema>()`](https://typia.io/docs/llm/parameters/) specifying the `IMember` type, then call the `AutoViewAgent.generate()` function. The TypeScript code will be generated and available in the `IAutoViewResult.transformTsCode` property.
...
Note that the [`typia.llm.parameters<Schema>()`](https://typia.io/docs/llm/parameters/) function only supports static object types without dynamic keys. If you need to generate frontend code for non-object types like `Array<IMember>`, you must use the [`typia.llm.schema<Schema>()`](https://typia.io/docs/llm/schema/) function instead.

When generating a schema with the [`typia.llm.schema<Schema>()`](https://typia.io/docs/llm/schema/) function, it's crucial to pre-define and assign a `$defs` variable of type `Record<string, ILlmSchema>`.
...
const func: IHttpLlmFunction | undefined = app.functions.find(
  (func) =>
