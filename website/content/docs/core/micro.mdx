...
`MicroAgentica` is a facade class of micro AI agent, which is a lightweight version of `Agentica` that is suitable for small-scale applications. Its [#Orchestration](/docs/concepts/function-calling/#orchestration-strategy) does not have a selector phase, but has the caller and describer phases. `MicroAgentica` directly lists every function to the caller.

If your number of functions is less than 8, `MicroAgentica` is recommended. As there is no [selector phase](/docs/concepts/function-calling/#orchestration-strategy), `MicroAgentica` is faster than `Agentica`. If your catalog is larger, use `Agentica` with [`config.selector`](/docs/core/selector), because the caller model tends to lose accuracy when every function is listed at once.
...
As `MicroAgentica` does not have a selector phase, its event types are narrower than `Agentica`. It emits `userMessage`, `assistantMessage`, `call`, `execute`, `describe`, `request`, `response`, `validate`, and `jsonParseError`.
