...
When you want to serve functions from a Swagger or OpenAPI document, create an LLM application schema from `assertHttpLlmApplication()` function (or `HttpLlm.application()` function). And provide the connection information to the HTTP server.
...
<YoutubeRenderer src="https://www.youtube.com/embed/RAzYo02HTXA" />
...
## Authorization
```typescript filename="src/main.ts" showLineNumbers {25-30}
import {
  Agentica,
  IAgenticaController,
  assertHttpLlmApplication
...
`assertHttpLlmApplication()` embeds [#Validation Feedback Strategy](/docs/concepts/function-calling/#validation-feedback).
