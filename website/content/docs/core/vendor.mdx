...
`Agentica` is utilizing OpenAI SDK (`npm i openai`) for LLM (Large Language Model) service interaction. However, it does not mean you can use only OpenAI GPT models. The OpenAI SDK is just a connection tool to the LLM vendor's API, and as most modern LLM vendors provide OpenAI-compatible APIs, you can use other vendors like Claude, DeepSeek, and so on.
...
OpenAI's JSON schema model is based on JSON schema 2020-12 draft (OpenAPI v3.1 is following), but some constraint properties like `IJsonSchema.IString.format` and `IJsonSchema.IInteger.minimum` are not supported in the OpenAI schema model `IChatGptSchema`.
