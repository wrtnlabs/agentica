...
Conversion of OpenAPI Specification to LLM Function Calling Schema.

LLM function calling needs JSON schema based function schema. However, service vendors of LLM (Large Language Model) are not using the same specified JSON schema. "OpenAI GPT" and "Anthropic Claude" are using different JSON schema specifications for LLM function calling, and Google Gemini is also different.
...
In the shopping mall benchmark, smaller OpenAI models have historically produced many type-level mistakes when filling function-calling arguments. To overcome the imperfection of LLM function calling, `@agentica` uses validation feedback strategy.
...
And `@agentica` enters a loop until candidate functions are empty. In the loop, the `caller` agent tries LLM function calling by analyzing the user's conversation text. If the context is enough to compose arguments, the `caller` agent calls the target functions and lets the `describer` agent explain the results. Otherwise, the `caller` agent requests more information from the user.
...
In current `@agentica/core`, the selector phase can be configured with [`config.selector`](/docs/core/selector). The default `llm` mode preserves the full-catalog LLM selector. `local`, `hybrid`, and `auto` modes use `AgenticaOperationIndex` to reduce candidate operations before execution.
