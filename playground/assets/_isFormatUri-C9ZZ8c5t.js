...
If you can't find any proper function to select, just type your own message. By the way, when typing your own message, please consider the user's language locale code. If your message is different with the user's language, please translate it to the user's.`},Zr={};class sr{constructor(t){this.closure_=t,this.value_=Zr}get(...t){return this.value_===Zr&&(this.value_=this.closure_(...t)),this.value_}}var rp={};const ap=new sr(()=>{const e=t=>typeof t=="object"&&t!==null;return typeof global=="object"&&e(global)&&e(global.process)&&e(global.process.versions)&&typeof global.process.versions.node<"u"}),op=new sr(()=>{var e;return ap.get()?((e=rp.LANG)==null?void 0:e.split(".")[0])??"en-US":navigator.language}),sp=new sr(()=>Intl.DateTimeFormat().resolvedOptions().timeZone);function up(e){var i,r;if(((i=e==null?void 0:e.systemPrompt)==null?void 0:i.common)!==void 0)return(r=e==null?void 0:e.systemPrompt)==null?void 0:r.common(e);const t=(e==null?void 0:e.locale)??op.get(),n=(e==null?void 0:e.timezone)??sp.get();return _t.COMMON.replace("${locale}",t).replace("${timezone}",n)}const jt={write:up};class lp{constructor(){this.queue=[],this.resolvers=[],this.closeResolvers=[],this.emptyResolvers=[],this.closed=!1}enqueue(t){var n;this.queue.push(t),this.resolvers.length>0&&((n=this.resolvers.shift())==null||n({value:this.queue.shift(),done:!1}))}async dequeue(){return this.queue.length>0?{value:this.queue.shift(),done:!1}:this.closed?(this.emptyResolvers.length>0&&(this.emptyResolvers.forEach(t=>t()),this.emptyResolvers=[]),{value:void 0,done:!0}):new Promise(t=>this.resolvers.push(t))}isEmpty(){return this.queue.length===0}isClosed(){return this.closed}done(){return this.isClosed()&&this.isEmpty()}close(){var t;for(this.closed=!0;this.resolvers.length>0;)(t=this.resolvers.shift())==null||t({value:void 0,done:!0});this.closeResolvers.forEach(n=>n())}async waitUntilEmpty(){return this.isEmpty()?Promise.resolve():new Promise(t=>{this.emptyResolvers.push(t)})}async waitClosed(){return this.isClosed()?Promise.resolve():new Promise(t=>{this.closeResolvers.push(t)})}}class Ho{constructor(){this.queue=new lp,this.consumer=new ReadableStream({pull:async t=>{const{value:n,done:i}=await this.queue.dequeue();if(i===!0){t.close();return}t.enqueue(n)}})}produce(t){this.queue.enqueue(t)}close(){this.queue.close()}done(){return this.queue.done()}async waitClosed(){await this.queue.waitClosed()}async waitUntilEmpty(){await this.queue.waitUntilEmpty()}}async function cp(e,t){var o,u,l;if(t.length===0)return[];const n=await e.request("describe",{messages:[{role:"system",content:jt.write(e.config)},...t.map(Ut).flat(),{role:"system",content:((l=(u=(o=e.config)==null?void 0:o.systemPrompt)==null?void 0:u.describe)==null?void 0:l.call(u,t))??_t.DESCRIBE}]}),i=[],r=await Qe.reduce(n,async(c,p)=>{const d=await c,g=m=>{for(const E of m){if(E.finish_reason!=null){i[E.index].mpsc.close();continue}if(E.delta.content==null)continue;if(i[E.index]!=null){i[E.index].content+=E.delta.content,i[E.index].mpsc.produce(E.delta.content);continue}const _=new Ho;i[E.index]={content:E.delta.content,mpsc:_},_.produce(E.delta.content),e.dispatch(np({executes:t,stream:_.consumer,done:()=>_.done(),get:()=>{var M;return((M=i[E.index])==null?void 0:M.content)??""},join:async()=>(await _.waitClosed(),i[E.index].content)}))}};return d.object==="chat.completion.chunk"?(g([d,p].flatMap(m=>m.choices)),rt.merge([d,p])):(g(p.choices),rt.accumulate(d,p))});if(r==null)throw new Error("No completion received");return r.choices.map(c=>c.message.role==="assistant"?c.message.content:null).filter(c=>c!==null).map(c=>wo({executes:t,text:c}))}async function ni(e,t){const n=e.stack.findIndex(r=>r.operation.name===t.name);if(n===-1)return null;const i=e.stack[n];return e.stack.splice(n,1),await e.dispatch(Go({selection:Ot({operation:i.operation,reason:t.reason})})),i}const pp={functions:[{name:"cancelFunctions",parameters:{description:` Properties of the function
...
called from the backend server, utilize this function too.`,validate:(()=>{const e=a=>!0,t=(a,o,u=!0)=>!0,n=a=>typeof a=="object"&&a!==null&&Array.isArray(a)===!1&&e();let i,r;return a=>{if(n(a)===!1){i=[],r=bn(i),((u,l,c=!0)=>(typeof u=="object"&&u!==null&&Array.isArray(u)===!1||r(!0,{path:l+"",expected:"object",value:u}))&&t(u,l+"",!0)||r(!0,{path:l+"",expected:"object",value:u}))(a,"$input",!0);const o=i.length===0;return o?{success:o,data:a}:{success:o,errors:i,data:a}}return{success:!0,data:a}}})()}][0];async function yp(e){var a,o,u;const t=await e.request("initialize",{messages:[{role:"system",content:jt.write(e.config)},...e.histories.map(Ut).flat(),{role:"user",content:e.prompt.text},{role:"system",content:((u=(o=(a=e.config)==null?void 0:a.systemPrompt)==null?void 0:o.initialize)==null?void 0:u.call(o,e.histories))??_t.INITIALIZE}],tools:[{type:"function",function:{name:vn.name,description:vn.description,parameters:vn.parameters}}],tool_choice:"auto",parallel_tool_calls:!1}),n=[],i=await Qe.reduce(t,async(l,c)=>{const p=await l,d=g=>{var m;for(const E of g){if(E.finish_reason!=null){(m=n[E.index])==null||m.mpsc.close();continue}if(E.delta.content==null)continue;if(n[E.index]!=null){n[E.index].content+=E.delta.content,n[E.index].mpsc.produce(E.delta.content);continue}const _=new Ho;n[E.index]={content:E.delta.content,mpsc:_},_.produce(E.delta.content),e.dispatch(ti({role:"assistant",stream:_.consumer,done:()=>_.done(),get:()=>n[E.index].content,join:async()=>(await _.waitClosed(),n[E.index].content)}))}};return p.object==="chat.completion.chunk"?(d([p,c].flatMap(g=>g.choices)),rt.merge([p,c])):(d(c.choices),rt.accumulate(p,c))});if(i===null)throw new Error("No completion received");const r=[];for(const l of i.choices)l.message.role==="assistant"&&l.message.content!=null&&r.push(Sn({role:"assistant",text:l.message.content}));return i.choices.some(l=>l.message.tool_calls!=null&&l.message.tool_calls.some(c=>c.type==="function"&&c.function.name===vn.name))&&await e.initialize(),r}async function Uo(e,t){const n=e.operations.flat.get(t.name);if(n===void 0)return null;const i=Ot({operation:n,reason:t.reason});return e.stack.push(i),e.dispatch(ep({selection:i})),n}const bp={functions:[{name:"selectFunctions",parameters:{description:` Properties of the function
