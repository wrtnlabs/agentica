...
상태: 2026-05-25에 1차 opt-in result budget을 구현했다. `IAgenticaConfig.context.resultBudget`과 `decodeHistories()`가 들어갔고, Agentica의 initialize/select/cancel/call/describe request가 이를 사용한다. Public history와 `MicroAgentica` path는 변경하지 않았다. 아직 persistence가 있는 `AgenticaResultStore`, digest/version, segmented read는 후속이다.
