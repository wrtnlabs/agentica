...
## Compact보다 넓은 Context Projection

Claude Code의 context 최적화는 `/compact` 하나가 아니다. 실제 순서는 대체로 다음 계층이 함께 움직인다.

1. schema/tool loading을 늦춘다.
2. 큰 결과를 preview/reference로 materialize한다.
3. 큰 content를 line/page/item/jsonPath/offset 단위로 다시 읽게 한다.
4. 오래된 결과를 microcompact한다.
5. 그래도 context가 넘치면 full compact와 reactive retry를 수행한다.

따라서 Agentica에서 compact를 구현할 때도 `AgenticaContextProjector`, `AgenticaResultStore`, `AgenticaSegmentedContentReader`를 별도 책임으로 둔다. Compact는 이들이 만든 refs와 segment metadata를 summary/restore transaction에 포함하는 상위 lifecycle이다.
...
### Progressive Read와 Segmented Content

Claude Code의 파일 분할 읽기는 Agentica core에 filesystem dependency를 넣자는 뜻이 아니다. 큰 controller/API/resource result를 안정적인 slice로 다시 읽을 수 있게 만들자는 뜻이다.
...
- HTTP/OpenAPI response page
- class controller의 큰 JSON array/object
- MCP resource 또는 remote connector output
- search/list/read operation result
- task/background output log
- binary/blob result의 metadata와 text preview

projection 정책:

- 처음에는 `preview + fullRef + range + truncated`만 model context에 넣는다.
- 후속 turn에서 필요한 range만 재투영한다.
- 같은 `digest` 또는 `version`이면 전체 재주입 대신 unchanged stub을 넣는다.
- search/list 계열은 `content`, `files`, `count` 같은 mode와 cap/offset을 명시한다.
- adapter가 full persistence를 제공하지 않으면 preview-only fallback을 쓰고, compact summary에는 full 복원이 불가능하다는 사실을 남긴다.
...
- segmented content reference 목록
- task output cursor와 digest/version
...
- result store refs와 segmented content refs
- task output cursor, last notified output offset
