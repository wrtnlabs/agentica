...
2026-05-24 재확인 결과 OpenAI 문서도 `tool_search`를 공식 guide로 제공한다. Function Calling guide는 tool schema가 많거나 클 때 tool search와 함께 rarely-used tool loading을 defer할 수 있다고 설명하며, 현재 문서상 지원 모델은 `gpt-5.4` 이상으로 제한된다. 따라서 "provider-native deferred schema loading은 없다"는 과거 표현은 더 이상 일반 명제로 쓰면 안 된다. 다만 Agentica core 기본 설계는 여전히 provider-neutral local selector가 맞다.
...
- OpenAI `tool_search`가 있더라도 Agentica는 OpenAI/Anthropic/OpenRouter/local model을 모두 지원하는 OpenAI-compatible facade이므로, no-network/no-provider-native fallback local index를 유지해야 한다.
