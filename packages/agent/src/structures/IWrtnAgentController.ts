 ...
 *
 * `IWrtnAgentController` is a type represents a controller of the
 * {@link WrtnAgent}, which serves a set of functions to be called
 * by A.I. chatbot from LLM function calling.
 *
 * Also, `IWrtnAgentController` is an union type which can specify
 * a subtype by checking the {@link protocol} property.
 *
 * - HTTP server: {@link IWrtnAgentController..IHttp}
 * - TypeScript class: {@link IWrtnAgentController.IClass}
 *
 * @author Samchon
 */
export type IWrtnAgentController =
  | IWrtnAgentController.IHttp
  | IWrtnAgentController.IClass;
export namespace IWrtnAgentController {
  /**
   * HTTP controller.
   *
