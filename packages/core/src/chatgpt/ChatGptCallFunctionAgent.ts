...
async function execute<Model extends ILlmSchema.Model>(ctx: AgenticaContext<Model>): Promise<AgenticaPrompt<Model>[]> {
  ...
  for (const choice of completion.choices) {
    for (const tc of choice.message.tool_calls ?? []) {
      if (tc.type === "function") {
        ...
        closures.push(
          ...
          > => {
            const call: AgenticaCallEvent<Model> = AgenticaEventFactory.createCallEvent({
              id: tc.id,
              operation,
            ...
            void ctx.dispatch(
              AgenticaEventFactory.createExecuteEvent({
                id: call.id,
                operation: call.operation,
            ...
            return [
              execute,
              AgenticaPromptFactory.createCancelPrompt({
                id: call.id,
                selections: [
    ...
    if (
      ...
      closures.push(async () => {
        const value: AgenticaTextPrompt = AgenticaPromptFactory.createTextPrompt({
          role: "assistant",
...
async function propagate<Model extends ILlmSchema.Model>(ctx: AgenticaContext<Model>, call: AgenticaCallEvent<Model>, retry: number): Promise<AgenticaExecutePrompt<Model>> {
  if (call.operation.protocol === "http") {
    ...
    if (check.success === false) {
      void ctx.dispatch(
        AgenticaEventFactory.createValidateEvent({
          id: call.id,
          operation: call.operation,
          result: check,
        }),
    ...
    try {
      ...
      return (
        (success === false
          ...
          : null)
        ?? AgenticaPromptFactory.createExecutePrompt({
          operation: call.operation,
          id: call.id,
          arguments: call.arguments,
          value: response,
...
async function correct<Model extends ILlmSchema.Model>(ctx: AgenticaContext<Model>, call: AgenticaCallEvent<Model>, retry: number, error: unknown): Promise<AgenticaExecutePrompt<Model> | null> {
  ...
  return propagate(
    ctx,
    AgenticaEventFactory.createCallEvent({
      id: toolCall.id,
      operation: call.operation,
