...
### shoppings_customers_sales_index
  ...
  - Reason: User wants to see every sales available in the shopping mall, so we need to retrieve a list of all summarized sales.
...
### shoppings_customers_sales_at
  ...
  - Reason: Retrieve detailed information about the MacBook sale following the user's request after listing all sales.
...
### shoppings_customers_orders_create
  ...
  - Reason: We then need to create a new order application from the shopping cart with the selected MacBook stock before publication.
...
### shoppings_customers_orders_publish_create
  ...
  - Reason: Finally, we publish the order with cash payment details provided, so we need to publish the order.
